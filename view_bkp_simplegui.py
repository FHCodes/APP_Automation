from masterfile import MasterfileCreator, cria_zip, create_folder, delete_folder
...
class GeraInferface:
    ...
    def gera_interface():    
        ...
        janela = sg.Window("APP AUTOMATION - Versão 4.1", tab_group, icon='icons/esse.ico')
        ...
        while True:
            ...
            if evento == "Criar":
                ...
                ziper = True
                path_name_created = create_folder(valores["path_name"])
                masterfile_creator = MasterfileCreator(valores["path_name"], valores["inventory_name"].split('\n'),valores["Vmf"],valores["caminho_master"], path_name_created)
                result = masterfile_creator.create_masterfiles()
                if not result:
                    delete_folder(path_name_created)
        
            if evento == "Renomear":
                RenameFiles.renomeia_arquivos(valores["path_name2"])
            
            if evento == "Gerar DBN0":
                DBN0Model.modelo_DBN0(valores["lista_DBN0"].split('\n'), valores["schema_DBN0"].rstrip('\n'),valores["path_DBN0"])

            if evento == "Gerar DBN1":
                DBN1Model.modelo_DBN1(valores["lista_DBN1"].split('\n'), valores["schema_DBN1"].rstrip('\n'),valores["path_DBN1"],valores["classe_DBN1"])
